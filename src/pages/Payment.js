...
function CardPreview({ number, holder, expiry, isFlipped, cardType }) {
  const fmt = (n) => {
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            {/* show mastercard icon when applicable */}
            {cardType === 'mastercard' ? (
              <SiMastercard size={26} color="rgba(255,255,255,0.85)" />
            ) : (
...
export default function Payment() {
  ...
  // keep cardType synced if user pastes number or changes programmatically
  useEffect(() => {
    setCardType(detectCard(form.number));
  }, [form.number]);
  ...
  const handleNumber = (e) => {
    ...
    // keep displayed formatting for user convenience
    e.target.value = fmt;
  };
  ...
  const validate = () => {
    ...
    if (!/^\d{2}\/\d{2}$/.test(form.expiry)) errs.expiry = 'Format: MM/YY';
      ...
      const now = new Date();
      const exp = new Date(2000 + yy, mm - 1 + 1, 1); // end of month safe check
  ...
  if (succeeded) {
    return (
      <div className="page page-payment" role="main" aria-live="polite" style={{ display: 'flex', justifyContent: 'center' }}>
  ...
  // ── Payment form ────────────────────────────────────────
  return (
    <div className="page page-payment" role="main" style={{ display: 'flex', justifyContent: 'center' }}>
      <div ref={wrapRef} style={{
