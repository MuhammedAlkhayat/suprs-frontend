...
import {
  FaParking, FaCar, FaCheckCircle, FaWallet,
  FaClock, FaFire, FaChartBar,
} from 'react-icons/fa';
...
function CountdownTimer({ checkIn, durationHours }) {
  ...
  useEffect(() => {
    const end = new Date(checkIn).getTime() + durationHours * 3600000;
    const tick = () => {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:8,
      ...
      border:`1px solid ${isExpiring ? 'rgba(239,68,68,0.3)' : 'rgba(0,210,255,0.2)'}` }}>
...
function OccupancyHeatmap({ slots }) {
  const hours = Array.from({ length: 24 }, (_, i) => i);
  // Simulated heat data (replace with real hourly booking data from backend)
  ...
  return (
    <div>
      <div style={{ display:'flex', gap:3, alignItems:'flex-end', height:40 }}>
        {heat.map((v, i) => (
          <div key={i} title={`${i}:00 — ${Math.round(v*100)}% occupancy`}
            style={{ flex:1, height:`${v*100}%`, minHeight:4,
              background: color(v), borderRadius:2,
...
export default function Dashboard() {
  ...
  const myActiveBooking = bookings.find(b => b.status === 'ACTIVE' && b.user_id === user?.id);
  const mySlot = myActiveBooking ? slots.find(s => s.id === myActiveBooking.slot_id) : null;

  const STATS = [
    { label:'Available', value: available, icon:<FaCheckCircle/>, color:'#10b981', bg:'rgba(16,185,129,0.12)', cls:'green' },
    { label:'Occupied',  value: occupied,  icon:<FaCar/>,         color:'#ef4444', bg:'rgba(239,68,68,0.12)',  cls:'red' },
    { label:'Reserved',  value: reserved,  icon:<FaParking/>,     color:'#f59e0b', bg:'rgba(245,158,11,0.12)', cls:'yellow' },
    { label:'Total Slots',value: total,    icon:<FaChartBar/>,    color:'#00d2ff', bg:'rgba(0,210,255,0.12)',  cls:'primary' },
  ];
  ...
  return (
    <div ref={containerRef}>
      ...
      {myActiveBooking && mySlot && (
        <div className="dash-anim glass-card glow-primary" style={{ marginBottom:24, padding:'1.2rem 1.5rem' }}>
          <div style={{ display:'flex', flexWrap:'wrap', gap:16, alignItems:'center', justifyContent:'space-between' }}>
            <div style={{ display:'flex', alignItems:'center', gap:12 }}>
              <div style={{ width:44, height:44, borderRadius:'var(--radius)',
                background:'rgba(0,210,255,0.12)', display:'flex', alignItems:'center',
              ...
              <div>
                ...
                <div style={{ fontSize:12, color:'var(--text-muted)' }}>
                  {myActiveBooking.duration_hours}h · ${Number(myActiveBooking.total_amount||0).toFixed(2)} ·{' '}
                  <span style={{ color: myActiveBooking.payment_status === 'PAID' ? '#10b981' : '#f59e0b', fontWeight:600 }}>
      ...
      {/* Stats */}
      <div className="dash-anim stagger-children"
      ...
      {/* Occupancy bar + heatmap */}
      <div className="dash-anim" style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))', gap:20, marginBottom:28 }}>
        {/* Occupancy */}
        <div className="glass-card" style={{ padding:'1.4rem' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
            <h5 style={{ margin:0, fontWeight:700, fontSize:14 }}>🔥 Occupancy Rate</h5>
            <span style={{ fontSize:22, fontWeight:900,
              color: occupancyPct >= 80 ? '#ef4444' : occupancyPct >= 50 ? '#f59e0b' : '#10b981' }}>
              {occupancyPct}%
            </span>
          </div>
        ...
        <div className="glass-card" style={{ padding:'1.4rem' }}>
          ...
          <OccupancyHeatmap slots={slots} />
          <div style={{ display:'flex', gap:12, marginTop:10, fontSize:10, color:'var(--text-muted)' }}>
            {[['#10b981','Low'],['#3b82f6','Medium'],['#f59e0b','High'],['#ef4444','Peak']].map(([c,l]) => (
      ...
      <div className="dash-anim glass-card">
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20, flexWrap:'wrap', gap:10 }}>
          ...
          <button className="btn-suprs" onClick={() => navigate('/booking')}
        ...
        <div className="parking-grid">
          {slots.map(slot => {
            const st = slot.status?.toLowerCase() || 'available';
            return (
              <div key={slot.id} className={`slot-box ${st}`}
                onClick={() => st === 'available' && navigate('/booking')}>
                <div style={{ fontSize:22 }}>
                  {st === 'available' ? '🟢' : st === 'occupied' ? '🔴' : '🟡'}
                </div>
                <div className="slot-number">{slot.slot_number}</div>
                <div className={`slot-status-badge ${st}`}>{slot.status}</div>
                {slot.price_per_hour && (
                  <div className="slot-meta">${slot.price_per_hour}/hr</div>
                )}
                {st === 'occupied' && slot.occupant_email && (
                  <div className="slot-meta" style={{ fontSize:9 }}>
                    {slot.occupant_email.split('@')[0]}
                  </div>
                )}
              </div>
            );
